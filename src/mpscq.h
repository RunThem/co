 ...
 * first to last. */
static inline void mpsc_queue_insert_list(struct mpsc_queue* queue,
...
static inline void mpsc_queue_insert_batch(struct mpsc_queue* queue,
                                           size_t n_nodes,
...
#define MPSC_QUEUE_FOR_EACH(node, queue)                                                           \
  for (node = mpsc_queue_tail(queue); node != NULL; node = mpsc_queue_next((queue), node))

#define MPSC_QUEUE_FOR_EACH_POP(node, queue) while ((node = mpsc_queue_pop(queue)))
...
static inline void mpsc_queue_push_front(struct mpsc_queue* queue, struct mpsc_queue_node* node);

static inline enum mpsc_queue_poll_result mpsc_queue_poll(struct mpsc_queue* queue,
                                                          struct mpsc_queue_node** node);

static inline struct mpsc_queue_node* mpsc_queue_pop(struct mpsc_queue* queue);

static inline struct mpsc_queue_node* mpsc_queue_tail(struct mpsc_queue* queue);

static inline struct mpsc_queue_node* mpsc_queue_next(struct mpsc_queue* queue,
...
static inline void mpsc_queue_insert_batch(struct mpsc_queue* queue,
  ...
  for (size_t i = 0; i < n_nodes - 1; i++) {
    node = node_ptrs[i];
    atomic_store_explicit(&node->next, node_ptrs[i + 1], memory_order_relaxed);
  }
...
static inline bool mpsc_queue_is_empty(struct mpsc_queue* queue) {
  struct mpsc_queue_node* tail;
  ...
  return (tail == &queue->stub && next == NULL && tail == head);
}
